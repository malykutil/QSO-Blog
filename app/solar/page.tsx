...
import { SolarSummaryCards } from "@/app/components/solar-summary-cards";
...
const temperatureLabels = [["object_temperature", "Objekt"], ["battery_temperature", "Baterie"], ["outside_temperature", "Venku"], ["mppt_temperature", "MPPT"]] as const;
const currentSeries = [["solar1_current", "Solár 1", "#f59e0b"], ["solar2_current", "Solár 2", "#38bdf8"], ["battery_current", "Baterie", "#4ade80"]] as const;
const temperatureSeries = [["object_temperature", "Objekt", "#f97316"], ["battery_temperature", "Baterie", "#a855f7"], ["outside_temperature", "Venku", "#38bdf8"], ["mppt_temperature", "MPPT", "#ef4444"]] as const;
...
export default function SolarPage() {
  ...
  return <AppShell><div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
    ...
    <section className="glass-panel rounded-[2rem] p-6 md:p-8"><p className="text-xs uppercase tracking-[0.35em] text-slate-500">Teploty a vlhkost</p><h2 className="mt-3 text-3xl font-semibold text-slate-950">Stav systému</h2><div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">{temperatureLabels.map(([key, label]) => <div className="rounded-[1.5rem] bg-slate-100/80 p-5" key={key}><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-3xl font-semibold text-slate-950">{value(telemetry?.[key], "°C")}</p></div>)}<div className="rounded-[1.5rem] bg-sky-50 p-5"><p className="text-sm text-sky-700">Objekt — vlhkost</p><p className="mt-2 text-3xl font-semibold text-sky-950">{value(telemetry?.object_humidity, "%")}</p></div></div><div className="mt-4 grid gap-4 md:grid-cols-3"><div className="rounded-[1.5rem] bg-cyan-50 p-5"><p className="text-sm text-cyan-700">Venkovní tlak</p><p className="mt-2 text-3xl font-semibold text-cyan-950">{value(telemetry?.outside_pressure, "hPa")}</p></div><div className="rounded-[1.5rem] bg-slate-100/80 p-5"><p className="text-sm text-slate-500">MQ-9 analogový signál</p><p className="mt-2 text-3xl font-semibold text-slate-950">{value(telemetry?.mq9_voltage, "V")}</p><p className="mt-2 text-xs text-slate-500">RAW: {value(telemetry?.mq9_raw, "")}</p></div><div className="rounded-[1.5rem] bg-amber-50 p-5"><p className="text-sm text-amber-700">MQ-9 stav</p><p className="mt-2 text-sm font-semibold text-amber-950">{telemetry?.mq9_raw !== null && telemetry?.mq9_raw !== undefined ? "Signál dostupný" : "Čekám na ADC"}</p></div></div></section>
