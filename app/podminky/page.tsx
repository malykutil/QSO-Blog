...
export default function PodminkyPage() {
  ...
  const [bandInput, setBandInput] = useState("20m");
  ...
  const pskMapLink = useMemo(() => {
    const call = encodeURIComponent((activeQuery.callsign || "OK2MKJ").trim().toUpperCase());
    return `https://pskreporter.info/pskmap.html?callsign=${call}`;
  }, [activeQuery.callsign]);
  ...
  const loadPsk = useCallback(async (query: PskQuery) => {
    ...
    if (!response.ok) {
      ...
      setError(body?.error ?? "PSK data nejdou nacist.");
      setLoading(false);
      return;
    }
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        <section className="relative overflow-hidden rounded-[2.4rem] border border-slate-900/8 bg-[linear-gradient(135deg,_#0a1420_0%,_#14314c_42%,_#1f5d8f_100%)] p-7 text-white shadow-[0_24px_80px_rgba(13,27,50,0.16)] md:p-9">
          ...
          <div className="relative">
            <p className="text-xs uppercase tracking-[0.35em] text-sky-100/70">Podminky vysilani</p>
            <h1 className="mt-3 font-display text-5xl leading-tight">HamSolar + PSK Reporter</h1>
            <p className="mt-4 max-w-3xl text-lg leading-8 text-sky-50/80">
              Nahore solarni stav, pod nim tvoje PSK nastaveni a hned pod tim mapa, kde je tvoje stanice slyset.
            </p>
          </div>
        </section>
        ...
        <section className="glass-panel rounded-[2rem] p-6 md:p-8">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-semibold text-slate-950">Aktualni podminky</h2>
            </div>
            <a
              href="https://www.hamqsl.com/solar.html"
              target="_blank"
              rel="noreferrer"
              className="rounded-full border border-slate-900/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-700 transition hover:bg-slate-100"
            >
              Otevrit detail
            </a>
          </div>
        ...
        <section className="glass-panel rounded-[2rem] p-6 md:p-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-semibold text-slate-950">Nastaveni filtru</h2>
            </div>
            <a
              href={pskMapLink}
              target="_blank"
          ...
          <form onSubmit={handleSubmit} className="mt-6 grid gap-3 rounded-[1.4rem] bg-slate-100/80 p-4 md:grid-cols-3">
            ...
            >
              {loading ? "Nacitam spots..." : "Pouzit nastaveni"}
            </button>
          </form>
          ...
          {!error && payload && payload.count === 0 ? (
            <p className="mt-4 rounded-[1.1rem] border border-amber-300/30 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              Pro zadanou volacku a interval nebyly nalezeny zadne PSK spoty. Zkus delsi cas (napr. 24 hodin) nebo jine pasmo.
            </p>
          ) : null}

          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            <div className="rounded-[1.2rem] border border-slate-900/8 bg-white/80 p-4">
              <p className="text-[11px] uppercase tracking-[0.25em] text-slate-500">Spotu</p>
            ...
            <div className="rounded-[1.2rem] border border-slate-900/8 bg-white/80 p-4">
              <p className="text-[11px] uppercase tracking-[0.25em] text-slate-500">Zeme</p>
        ...
        <section className="glass-panel rounded-[2rem] p-6 md:p-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Mapa slysetelnosti</p>
              <h2 className="mt-3 text-3xl font-semibold text-slate-950">Kde je stanice slyset</h2>
            </div>
            <p className="text-sm text-slate-500">
              Posledni aktualizace: {payload?.fetchedAt ? new Date(payload.fetchedAt).toLocaleString("cs-CZ") : "--"}
            </p>
          </div>
            ...
            <table className="w-full border-collapse text-left text-sm">
              <thead className="bg-slate-100 text-slate-600">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium">Lokator</th>
                  <th className="px-4 py-3 font-medium">Zeme</th>
