...
export function InteractiveHistoryChart({ history, series, title, unit }: { history: SolarTelemetry[]; series: ChartSeries; title: string; unit: string }) {
  ...
  return <div className="glass-panel rounded-[2rem] p-6 md:p-8"><div><p className="text-xs uppercase tracking-[0.35em] text-slate-500">Historie</p><h2 className="mt-3 text-3xl font-semibold text-slate-950">{title}</h2></div>{history.length < 2 ? <p className="mt-6 rounded-[1.2rem] bg-slate-100 px-4 py-4 text-sm text-slate-600">Pro graf potřebuji alespoň dvě měření z Raspberry Pi.</p> : <><div className="mt-6 overflow-hidden rounded-[1.5rem] bg-slate-950 p-3"><svg viewBox={`0 0 ${width} ${height}`} className="h-auto w-full" role="img" aria-label={`${title}. Najetím myší zobrazíte čas a hodnoty.`} onMouseLeave={() => setHoveredIndex(null)}><g stroke="#29404c" strokeWidth="1"><path d={`M0 ${padding}H${width}`} /><path d={`M0 ${height / 2}H${width}`} /><path d={`M0 ${height - padding}H${width}`} /></g>{min < 0 ? <path d={`M0 ${height - ((0 - min) / span) * plotHeight - padding}H${width}`} stroke="#94a3b8" strokeDasharray="5 5" /> : null}{series.map(([key, , color]) => <path key={String(key)} d={pathFor(key)} fill="none" stroke={color} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />)}{hoveredPoint && hoveredIndex !== null ? <><line x1={pointX(hoveredIndex)} x2={pointX(hoveredIndex)} y1={padding} y2={height - padding} stroke="#e2e8f0" strokeDasharray="4 5" />{series.map(([key, , color]) => <circle key={`point-${String(key)}`} cx={pointX(hoveredIndex)} cy={pointY(key, hoveredIndex)} r="5" fill={color} stroke="#f8fafc" strokeWidth="2" />)}<g transform={`translate(${tooltipX} 18)`}><rect width="218" height={34 + series.length * 22} rx="10" fill="#f8fafc" stroke="#cbd5e1" /><text x="12" y="21" fill="#0f172a" fontSize="13" fontWeight="700">{formatDate(hoveredPoint.recorded_at)}</text>{series.map(([key, label, color], seriesIndex) => <g key={String(key)} transform={`translate(12 ${45 + seriesIndex * 22})`}><circle cx="4" cy="-4" r="4" fill={color} /><text x="14" y="0" fill="#334155" fontSize="12">{label}: {formatValue(hoveredPoint[key], unit)}</text></g>)}</g></> : null}<rect x="0" y="0" width={width} height={height} fill="transparent" onPointerMove={handlePointerMove} /></svg></div><div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-600">{series.map(([, label, color]) => <span key={label} className="inline-flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: color }} />{label}</span>)}<span className="text-slate-400">{unit} · najeďte myší na graf</span></div></>}</div>;
}
