...
struct Telemetry { float batteryVoltage = NAN, batteryCurrent = NAN, solar1Power = NAN, solar2Power = NAN, solar1Current = NAN, solar2Current = NAN, loadPower = NAN, batteryTemp = NAN, objectTemp = NAN, outsideTemp = NAN, mpptTemp = NAN, objectHumidity = NAN, rpiCpuTemp = NAN; int solarEnergy = 0; String recordedAt; } telemetry;
...
void drawOverviewScreen() {
  ...
  card(RECT_OVERVIEW_POWER); label("CPU RPI", RECT_OVERVIEW_POWER); valueText(celsius(telemetry.rpiCpuTemp), RECT_OVERVIEW_POWER_VALUE, AMBER);
...
void drawDiagnosticsScreen() {
  ...
  label("WI-FI", RECT_DIAG_0); drawTextFit(WiFi.status() == WL_CONNECTED ? String(WiFi.RSSI()) + " dBm" : "offline", 10, 62, 140, TEXT, PANEL); drawTextFit("CPU RPI " + celsius(telemetry.rpiCpuTemp), 10, 86, 140, MUTED, PANEL);
